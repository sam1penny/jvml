...
def plot_individual_opts_code_size(plot_name : str, benchmark_names : list[str]) -> None:
    ...
    for benchmark_name in benchmark_results:
        for optimisation in benchmark_results[benchmark_name]:
            if optimisation != "jvml_unoptimised":
                ordered_benchmarks_by_size[optimisation].append(benchmark_results[benchmark_name][optimisation] / benchmark_results[benchmark_name]["jvml_unoptimised"])
    ...
    for optimisation, relative_execution_time in ordered_benchmarks_by_size.items():
        offset = width * multiplier
        ax.bar(x + offset, np.array(relative_execution_time)-1, width, bottom=1.0, label=optimisation)
        for i in range(len(relative_execution_time)):
            if relative_execution_time[i] == 0:
...
def plot_compiler_size(plot_name : str, benchmark_names : list[str]) -> None:
    ...
    for benchmark_name in benchmark_results:
        for compiler in benchmark_results[benchmark_name]:
            ordered_benchmarks_by_size[compiler].append(benchmark_results[benchmark_name][compiler])
    ...
    for compiler, relative_execution_time in ordered_benchmarks_by_size.items():
        offset = width * multiplier
...
def plot_varied_inlining_threshold(plot_name : str, benchmark_names : list[str]) -> None:
    ...
    for benchmark_name in benchmark_results:
        for threshold in benchmark_results[benchmark_name]:
            ordered_benchmarks_by_size[threshold].append(benchmark_results[benchmark_name][threshold])
    ...
    for threshold, relative_execution_time in ordered_benchmarks_by_size.items():
        offset = width * multiplier
...
if __name__ == "__main__":
    '''
    plot_peformance_graphs("comparing_compilers_time", [b[0] for b in benchmark_details])
